// Serverless payment — multi-chain. Redstring shares the studio wallet with
// Moonshot (same maker, same custody) so no new key material was created.
// The buyer sends ETH to PAY_ADDRESS on
// Base (an Ethereum L2 — fees under a cent, confirms in seconds) or Ethereum
// mainnet, pastes the transaction hash (or lets Redstring find it), and we
...
const USED_KEY = 'redstring_used_txs';
...
export async function verifyPayment(txHash) {
  ...
  if ((tx.to || '').toLowerCase() !== PAY_ADDRESS.toLowerCase()) return { ok: false, reason: 'That transaction wasn’t sent to the Redstring address.' };
